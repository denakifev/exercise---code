...
def display_hangman(tries):
    ...
    return stages[tries]
def check(l):
    n = 'ЁЙЦУКЕНГШЩЗХЪЭЖДЛОРПАВЫФЯЧСМИТЬБЮ'
    return l.isalpha() and len(l) == 1 and l in n 

def retake(mirrow, letter):
    nl =''
    for i in range(len(word)):
        if mirrow[i] == '-' and word[i] == letter:
            nl+=letter
        else:
            nl+=mirrow[i]  
    return nl 
...
def play():
    ...
    print()
    print(mirrow)
    print('Это загаданное слово',)
    print(f'У тебя осталось {tries} попыток')
    print(display_hangman(6))
    while tries != 0:
        letter = input('Введите букву ').upper()
        if check(letter) == False or letter in listoflet:
            print('Введите русскую БУКВУ, не названную ранее')
            continue
        else:
            listoflet.append(letter)
        if letter in word:
            mirrow = retake(mirrow, letter)
            print(mirrow)
        else:
...
if __name__ == '__main__':
    play()
    s = input('Вы хотите сыграть еще(да/нет) ')
    while s.lower().strip() == 'да':
        word = get_random()
        play()
